...
var _default = (api, {
  ...
  api.onDevCompileDone(a => {
    ...
    let readNum = 0; // const ignore = ['./pages']
    ...
    const filePath = path.resolve(entry); // 递归执行，直到判断是文件就执行readFileToObj
            ...
            fs.stat(fileDir, (err2, status) => {
              if (err2) {
                ...
                if (status.isDirectory()) {
                  ...
                  // 后缀不符合的跳过
                  if (!suffix.includes(path.extname(fileDir))) {// return
                  } else {
                    readNum++;
            ...
            fs.readFile(output, 'utf-8', (err, data) => {
              if (err) {
                ...
                try {
                  ...
                  // 翻车
                  console.warn(e); // errDeal(resolve).then(res => {
                  //   resolve()
                  // })
                }
              }
    ...
    function errDeal(resolve) {
      ...
      return new Promise(resolve2 => {
        resolve2(resolve); // const r1 = readline.createInterface({
        //   input: process.stdin,
